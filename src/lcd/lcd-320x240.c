...
#if 0
static void LCDBUS_Write(u16 val, int rs)
{
	if(rs) {
...
static void LCDBUS_WriteReg(u16 regn, u16 val)
{
...
// x=[0-240] , y=[0-320]
void LCD_SetPixel(unsigned int x, unsigned int y, u16 color)
{
	LCD_SetCursor(x,y);
	LCD_WriteRAM(color);
}
...
// x=[0-240] , y=[0-320]
u16 LCD_GetPixel(unsigned int x, unsigned int y)
{
	unsigned int addr;
	static volatile u16 RRR;
...
void LCD_WriteMem(unsigned int addr, const u16 *data, unsigned int nitems)
{
	LCDBUS_WriteReg(0x20, addr&0xff);	/* low addr */
	LCDBUS_WriteReg(0x21, (addr>>8)&0x1ff);	/* high addr */
...
void LCD_DrawCircle(int x, int y, int r, u16 color)
{
	int i, j;
...
/*-------------------------------------------------------------------------*/
static u16 LCD_ConBgColor;
static u16 LCD_ConFgColor;
